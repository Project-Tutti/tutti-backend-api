...
- AI 서버 URL 설정(`AI_SERVER_URL`)은 폐기되었으며, 현재는 **Redis Streams 통신**으로 대체되었습니다.
- K8s 클러스터 내부 또는 외부(Upstash)에 위치한 Redis를 공용 Message Broker로 사용합니다.
...
│  │              │                                           │    │
│  │   ┌──────────▼──────────────────────────────────────┐   │    │
│  │   │ ArrangementService → Redis XADD (ai:stream)     │   │    │
│  │   └──────────┬──────────────────────────────────────┘   │    │
│  └──────────────┼──────────────────────────────────────────┘    │
│                 │ Redis Streams                                 │
│                 ▼                                               │
...
| ②    | Main → Redis  | `XADD ai:arrange:stream`                | MIDI 메타데이터 + 매핑 정보 전달. Main은 즉시 클라이언트에게 응답 반환 |
| ③    | AI Worker     | `XREADGROUP block`                      | Redis에서 새로운 편곡 태스크 할당 받음                                |
| ④    | AI Worker     | 내부 처리                               | 백그라운드로 AI 추론 실행                                             |
...
1. **동기 호출 회피**: Main → AI HTTP 타임아웃 문제를 Redis Message Queue 도입을 통해 원천 차단
2. **크래시 레질리언스**: 작업 도중 AI Worker가 죽더라도 Redis XCLAIM 명세에 의해 자동 복구
3. **가비지 컬렉팅 방어**: Main 서버가 주기적으로 (15분) 응답 없는 태스크 식별 후 강제 취소
4. **진행률 중계**: AI 서버가 콜백으로 진행률을 보내면, Main 서버가 SSE로 클라이언트에 중계
